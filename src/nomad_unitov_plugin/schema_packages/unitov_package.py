...
class Unitov_SpinCoating_Recipe(SpinCoatingRecipe, EntryData):
    m_def = Section(
        a_eln=dict(hide=['lab_id', 'users']))


class Unitov_Chemical(Chemical, EntryData):
    m_def = Section(
        a_eln=dict(hide=['lab_id', 'users']))
...
class Unitov_Substrate(Substrate, EntryData):
    m_def = Section(
        a_eln=dict(
            hide=[
                'lab_id',
...
class Unitov_Solution(Solution, EntryData):
    m_def = Section(
        a_eln=dict(
            hide=[
...
class Unitov_Sample(SolcarCellSample, EntryData):
    m_def = Section(
...
class Unitov_Batch(Batch, EntryData):
    m_def = Section(
        a_eln=dict(
            hide=['users', 'samples'],
            properties=dict(
...
# %% ####################### Cleaning
class Unitov_Cleaning(Cleaning, EntryData):
    m_def = Section(
        a_eln=dict(
            hide=[
                'lab_id',
...
# %% ### Spin Coating
class Unitov_SpinCoating(SpinCoating, EntryData):
    m_def = Section(
        a_eln=dict(
            hide=[
                'lab_id',
...
class Unitov_SimpleMPPTracking(MPPTracking, EntryData):
    m_def = Section(
        a_eln=dict(
            hide=[
                'lab_id',
    ...
    def normalize(self, archive, logger):
        ...
        super(Unitov_SimpleMPPTracking,
              self).normalize(archive, logger)


class Unitov_MPPTracking(MPPTrackingHsprintCustom, PlotSection, EntryData):
    m_def = Section(
        a_eln=dict(
            hide=[
                'lab_id', 'solution',
...
class Unitov_EQEmeasurement(EQEMeasurement, EntryData):
    m_def = Section(
        a_eln=dict(
            hide=[
                'lab_id', 'solution',
    ...
    def normalize(self, archive, logger):
        ...
        super(Unitov_EQEmeasurement, self).normalize(archive, logger)
